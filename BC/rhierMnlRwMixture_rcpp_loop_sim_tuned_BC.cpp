...
double llmnl_BC(vec const& betastar, vec const& y, mat const& X, int const& pr){
  ...
  //This implementation is unproblematic as long as outside option is included to design (otherwise numerator & denominator can approach zero at same time) 
  for(int i = 0; i<n;i++){      
    for(int p = 0; p<j;p++){
    ...
    if (X(i*j+y[i]-1,pr_cpp) <= budget) {
      ...
      xby[i] = -1*datum::inf; //Insure zero likelihood if chosen alternative is outside budget
    }
  }
